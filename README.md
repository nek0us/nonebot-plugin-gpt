...
| gpt_render_mode | 否 | auto | auto/text/image | 富文本输出全局默认策略：`auto` 按内容和适配器能力选择，`text` 始终文本，`image` 会让不支持原生 Markdown 的适配器优先图片；支持原生富文本的适配器仍保持可读文本与可点击链接。可用“输出模式”在当前聊天范围单独覆盖，`默认`恢复此配置；渲染异常会回退文本。 |
...
| gpt_history_show_identity | 否 | true | bool | 历史聊天是否显示群聊发言者的稳定平台 ID。仅在未匿名化且该条消息带有群聊发言者标签时显示。 |
...
| gpt_agent_read_roots | 否 | `[]` | JSON List[Dict] | 向超级用户 Agent 提供命名的只读诊断目录，例如日志和源码。模型只能选择名称并填写根目录内相对路径；可列目录、查看元数据、搜索 UTF-8 文本、读取指定行或文件尾部，不跟随符号链接、不具备写入能力。每次读取或搜索需原聊天范围确认；只配置可以安全交给模型阅读的目录。 |
...
### markdown发送问题
插件不会把原始 Markdown 标记直接交给不支持它的平台。Telegram 适配器会转换为其原生富文本实体；其他适配器的复杂 Markdown、链接、表格和长内容会按 `gpt_render_mode` 转为兼容的纵向图片，普通内容仍尽量以文本发送。图片中的普通 Markdown 链接会编号并附带可复制的完整 URL；可通过 `gpt_chat_image_template` 调整聊天图片主题。
...
历史图片中的普通 Markdown 链接会显示为“标题[编号]”，图片页底部会列出对应标题和完整 URL，便于在图片中查看和手动复制；不会在图片发送后额外刷出一条参考链接文本。网页私有引用标记依赖上游元数据；旧历史未保存来源映射时会被隐藏，不会伪造出处。
