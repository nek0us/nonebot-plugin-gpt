...
`GPT帮助` 与 `工作状态` 会优先渲染为信息图片：前者按主题展示命令说明，后者展示账户概览和需要处理的状态。渲染环境异常时会自动回退为文本。黑白名单、人设列表、人设正文、历史聊天、历史会话、CDK 列表和智能体文本结果仍会自动按安全长度分页发送，避免在 OneBot、频道和 Telegram 等适配器中因单条消息过长而发送失败；分页文本会带有 `[页码/总页数]` 标记。
