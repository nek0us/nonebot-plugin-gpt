...
| 生成cdk [来源] | 兼容 | 仅超级管理员 | 否 | 任意会话 | 生成一次性跨平台白名单 CDK，并记录创建者、创建会话与可选来源备注 |
| 兑换 <CDK> | 兼容 | 无 | 否 | 群聊/私聊/频道 | 将当前访问范围加入白名单；兼容旧别名“出现吧”，每个 CDK 只能兑换一次 |
| cdk列表 / 作废cdk <CDK> | 兼容 | 仅超级管理员 | 否 | 任意会话 | 查看 CDK 来源和兑换范围，或作废未兑换的 CDK |
| 退出白名单 | 兼容 | 白名单 | 否 | 群聊/私聊/频道 | 撤销当前访问范围的白名单；兼容旧别名“结束吧” |
...
> 白名单、Plus 与 `gpt_manage_ids` 统一使用插件生成的访问范围标识。它会将同一群或频道的不同用户归为同一授权范围；旧版 `group/private/qqgroup/qqguild` 白名单文件不会自动转换为有效授权。

### CDK 授权

超级用户在任意会话执行 `生成cdk [来源备注]` 后，将机器人返回的 `兑换 <CDK>` 发送到目标群、私聊或频道即可完成授权。兑换码只可使用一次，自动绑定目标会话的跨平台访问范围标识，不需要手写群号或适配器类型。
...
工作状态、黑白名单、人设列表、人设正文、历史聊天、历史会话、CDK 列表和智能体文本结果会自动按安全长度分页发送。这样在 OneBot、频道和 Telegram 等适配器中都不会因单条消息过长而整条发送失败；分页文本会带有 `[页码/总页数]` 标记。
