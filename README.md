...
> 白名单分为会话授权与个人授权：会话授权使用访问范围标识，会将同一群或频道的不同用户归为同一授权范围；个人授权使用适配器命名空间加用户 ID，仅在同一适配器生效。旧版 `group/private/qqgroup/qqguild` 数据会保留兼容读取，其中旧版 `private` 白名单继续按 OneBot 用户在群聊和私聊中生效。

### CDK 授权

超级用户在任意会话执行 `生成cdk [来源备注]` 后，将机器人返回的 `兑换 <CDK>` 发送到目标群、私聊或频道即可完成会话授权。该兑换码只可使用一次，自动绑定目标会话的跨平台访问范围标识，不需要手写群号或适配器类型。

若需要给某一个人加白，使其在未授权群中也能正常提及机器人聊天，使用 `生成个人cdk [来源备注]`。目标用户在同一适配器任意私聊、群聊或频道发送 `兑换 <CDK>` 后，个人授权会绑定该用户；不同平台的用户 ID 不会互相冲突。
