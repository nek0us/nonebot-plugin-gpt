...
# strict：每个需确认步骤暂停；delegate：受限工作区读写、静态截图和图片回传自动执行；full：已注册工具自动执行。
...
`gpt_agent_approval_mode` 只影响超级用户入口，默认 `strict`：所有需要确认的工具都会暂停。`delegate` 适合日常制作网页、文件与截图：仅自动放行已明确标记为可委托的受限工作区读写、静态网页渲染和图片回传；脚本执行、系统命令、网络探测、受管服务、跨成员提醒及高风险变更仍逐次确认。`full` 会自动执行所有已注册工具，只应在完全自用、已审查工具配置的环境启用。

三档都不会取消工具白名单、工作区路径限制、参数校验、成员隔离或敏感任务预检；它们决定的是“已通过这些检查的工具是否仍要停下来等待确认”。
...
`strict` 下，所有写入、脚本执行、渲染和图片回传均要求原超级用户在原聊天范围确认。`delegate` 会自动处理受限工作区内的读写、静态渲染和图片回传，但脚本执行仍需确认；`full` 则按审批档位说明自动执行已注册工具。普通成员安全模式不会注册这些工具。
