...
.sheet { width: 760px; padding: 24px; }
.message { padding: 26px 28px; border: 1px solid #e2e5ef; border-radius: 14px; background: #ffffff; box-shadow: 0 6px 22px rgba(67, 76, 108, .08); }
.content { font-size: 17px; line-height: 1.78; overflow-wrap: anywhere; }
.content > :first-child { margin-top: 0; }.content > :last-child { margin-bottom: 0; }
h1, h2, h3 { color: #354064; letter-spacing: 0; }
h1 { margin: 24px 0 16px; padding-left: 14px; border-left: 6px solid #8a72d6; font-size: 28px; line-height: 1.3; }
h2 { margin: 22px 0 13px; padding-left: 12px; border-left: 5px solid #79a9dc; font-size: 23px; line-height: 1.35; }
h3 { margin: 19px 0 10px; padding-left: 10px; border-left: 4px solid #e58ab0; font-size: 19px; line-height: 1.4; }
...
table { width: 100%; margin: 15px 0; border-collapse: separate; border-spacing: 0; overflow: hidden; border: 1px solid #e0e4ee; border-radius: 9px; font-size: 15px; }
...
.sheet { width: 720px; padding: 25px 28px; }.content { font-size: 17px; line-height: 1.78; overflow-wrap: anywhere; }.content > :first-child { margin-top: 0; }.content > :last-child { margin-bottom: 0; }
h1 { margin: 22px 0 15px; padding-left: 13px; border-left: 5px solid #111111; font-size: 27px; } h2 { margin: 20px 0 12px; padding-left: 11px; border-left: 4px solid #333333; font-size: 22px; } h3 { margin: 17px 0 9px; padding-left: 9px; border-left: 3px solid #555555; font-size: 19px; }
...
table { width: 100%; margin: 15px 0; border-collapse: collapse; font-size: 15px; } th, td { padding: 9px 10px; border: 1px solid #222222; text-align: left; } th { background: #eeeeee; } a { color: #111111; text-decoration: underline; overflow-wrap: anywhere; } img { display: block; max-width: 100%; height: auto; margin: 14px auto; } hr { border: 0; border-top: 1px solid #333333; margin: 20px 0; }
...
def build_chat_html(markdown_text: str, *, template: str = "native") -> str:
    """将模型 Markdown 注入内置主题或用户提供的 HTML 模板。"""
    return _template_source(template).replace("{{ content }}", _markdown_to_html(markdown_text))


async def render_chat_markdown(markdown_text: str, *, template: str = "native") -> bytes:
    """渲染适合群聊阅读的窄幅纵向聊天图片。"""
    from nonebot_plugin_htmlkit import html_to_pic

    return await html_to_pic(
        build_chat_html(markdown_text, template=template),
        dpi=110,
        max_width=760,
        device_height=10,
